try {
    ...
    add_dislike_counts = () => {
        ...
        .then((response) => {
           ...
           waitElem('.YtDislikeButtonViewModelHost').then((dislikeView) => {
                dislikeView.querySelector('button').style.width = 'auto'
                dislikeView.querySelector('.yt-spec-button-shape-next__icon').style.marginRight = '6px'
                let dislikeText = document.createElement('div')
                dislikeText.classList.add('yt-spec-button-shape-next__button-text-content')
                dislikeText.textContent = formatNumber(dislike)
